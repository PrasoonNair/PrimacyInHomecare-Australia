...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Plan Analysis & Auto-Agreement Generation Endpoints
  app.get("/api/plan-analysis/:planId", isAuthenticated, async (req, res) => {
    try {
      const { planId } = req.params;
      const analysis = await storage.getPlanAnalysis(planId);
      res.json(analysis);
    } catch (error) {
  ...
  app.post("/api/plan-analysis/analyze", isAuthenticated, async (req, res) => {
    try {
      const { planId, participantId } = req.body;
      const analysis = await storage.analyzePlan({
        planId,
        participantId,
        analyzedBy: req.user?.id || 'current-user'
      });
      res.json(analysis);
    } catch (error) {
  ...
  app.get("/api/auto-agreements", isAuthenticated, async (req, res) => {
    try {
      const { planId } = req.query;
      const agreements = await storage.getAutoGeneratedAgreements(planId as string);
      res.json(agreements);
    } catch (error) {
      console.error("Error fetching auto-generated agreements:", error);
      res.status(500).json({ error: "Failed to fetch auto-generated agreements" });
    }
  });

  app.post("/api/auto-agreements/generate", isAuthenticated, async (req, res) => {
    try {
      const { analysisId } = req.body;
      const agreements = await storage.generateServiceAgreements(analysisId);
      res.json(agreements);
    } catch (error) {
      console.error("Error generating service agreements:", error);
      res.status(500).json({ error: "Failed to generate service agreements" });
    }
  });

  app.get("/api/agreement-communications", isAuthenticated, async (req, res) => {
    try {
      const { planId, agreementId } = req.query;
      const communications = await storage.getAgreementCommunications({
        planId: planId as string,
        agreementId: agreementId as string
      });
      res.json(communications);
    } catch (error) {
      console.error("Error fetching agreement communications:", error);
      res.status(500).json({ error: "Failed to fetch agreement communications" });
    }
  });

  app.post("/api/agreement-communications/send", isAuthenticated, async (req, res) => {
    try {
      const { agreementId, communicationMethod, recipientContact } = req.body;
      const communication = await storage.sendAgreementForSigning({
        agreementId,
        communicationMethod,
        recipientContact,
        sentBy: req.user?.id || 'current-user'
      });
      res.json(communication);
    } catch (error) {
      console.error("Error sending agreement for signing:", error);
      res.status(500).json({ error: "Failed to send agreement for signing" });
    }
  });

  app.get("/api/service-agreement-templates", isAuthenticated, async (req, res) => {
    try {
      const { supportCategory, serviceType } = req.query;
      const templates = await storage.getServiceAgreementTemplates({
        supportCategory: supportCategory as string,
        serviceType: serviceType as string
      });
      res.json(templates);
    } catch (error) {
      console.error("Error fetching service agreement templates:", error);
      res.status(500).json({ error: "Failed to fetch service agreement templates" });
    }
  });

  app.post("/api/service-agreement-templates", isAuthenticated, async (req, res) => {
    try {
      const templateData = req.body;
      const template = await storage.createServiceAgreementTemplate(templateData);
      res.json(template);
    } catch (error) {
      console.error("Error creating service agreement template:", error);
      res.status(500).json({ error: "Failed to create service agreement template" });
    }
  });

  app.get("/api/e-signature-integrations", isAuthenticated, async (req, res) => {
    try {
      const integrations = await storage.getESignatureIntegrations();
      res.json(integrations);
    } catch (error) {
      console.error("Error fetching e-signature integrations:", error);
      res.status(500).json({ error: "Failed to fetch e-signature integrations" });
    }
  });
